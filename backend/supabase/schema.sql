...
CREATE TABLE IF NOT EXISTS pageants (
    ...
    created_by UUID NOT NULL REFERENCES users (id),
    scoring_method TEXT NOT NULL CHECK (
        scoring_method IN (
            'average',
            'weighted',
            'ranking',
...
CREATE UNIQUE INDEX IF NOT EXISTS contestants_pageant_number_gender_scope_key ON contestants (
    pageant_id,
    contestant_number,
    COALESCE(gender, '__NO_GENDER__')
);
